...
int mwmAddPatternEx( void *pv, unsigned char * P, int m,
       unsigned noCase,  unsigned offset, unsigned depth , void *id, int iid ,rule_node_t* rule)
{
...
//#endif
	...
	//add by xingke.zhang 2013,05.20
	p->rule = rule;
...
int mwmAddPattern( void * pv, unsigned char * P, int m, unsigned id )
{
  ...
  //  modified by xingke.zhang 2013.05.20
    return mwmAddPatternEx( pv, P, m, 0, id, 0, pv, 0, NULL);
}
...
//             if( match( patrn->psID, (int)(T-Tx), data ) )
		     //modify by xingke.zhang, 2013.05.13
             if( match( patrn, (int)(T-Tx), Tx ) )
               return -(nfound+1);
           }
...
*/
int mwmMatch (  void* id, int index, void * data )
{
	MWM_PATTERN_STRUCT* p = (MWM_PATTERN_STRUCT*)id;
	rule_node_t* prule = (rule_node_t*)(p->rule);

	save_detect_log(prule);

   return 0;
}

//2013.05.08 add by xingke.zhang
/*
 * 向搜索引擎中加入规则库
 */
int mwmAddRule(void* engine, const rule_node_t* rule)
{
	if(mwmAddPatternEx(engine, (unsigned char*)rule->content, rule->content_len, rule->nocase, 0, 0, rule->id, 1, rule)<0)
		return -1;
